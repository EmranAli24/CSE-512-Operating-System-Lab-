...
\pagestyle{fancy}
\fancyhf{}
\lhead{CSE 512 (Operating System Lab)}
\rhead{Emran Ali \\ ID: 23701019}
\cfoot{\thepage}
...
%     \centering
%     \vspace*{2cm}
%     {\Huge\bfseries Operating Systems Lab\\Solution Manual\par}
%     \vspace{1.5cm}
%     {\Large Emran Ali\par}
%     {\large ID: 23701019\par}
%     \vspace{1cm}
...
\begin{titlepage}
    ...
    % Title at the top
    {\Large\bfseries\color{blue} \underline{Report Submission}\par}
    
    \vspace{2cm}
    
    % Left-aligned block for the details
    \begin{flushleft}
        \large
        \textbf{Student ID:} 23701019 \\
        \vspace{0.4cm}
        \textbf{Student Full Name:} Emran Ali \\
        \vspace{0.4cm}
        \textbf{Academic Semester: } Fifth Semester\\
        \vspace{0.4cm}
        \textbf{Session: } 2022-2023\\
        \vspace{0.4cm}
        \textbf{CSE Batch:} 23\\
        \vspace{0.4cm}
        \textbf{Department name:} Computer Science and Engineering \\
        \vspace{0.4cm}
        \textbf{Faculty Name:} Engineering \\
        \vspace{0.4cm}
        \textbf{Hall:} Shaheed Abdur Rab Hall \\
        \vspace{0.4cm}
        \textbf{University name:} University of Chittagong \\
        \vspace{0.4cm}
        \textbf{Course Code:} CSE 512 \\
        \vspace{0.4cm}
        \textbf{Course Title:} Operating Systems Lab \\
        \vspace{0.4cm}
        \textbf{Course Credits:} 3 \\
        \vspace{0.4cm}
...
% ========== QUESTION 1 ==========
\section{UNIX Permission and umask Calculator}
\subsection{Question}
...
% ========== QUESTION 2 ==========
\section{POSIX File Copy with open/read/write}
\subsection{Question}
...
% Duplicate sections with "Add your code here" have been updated to the same stacked format.

\section{Directory Listing and Metadata Report (ls + stat subset)}
\subsection{Question}
...
\section{grep-lite: Deterministic Text Pattern Search}
\subsection{Question}
...
% \noindent\textbf{Input:}
% \begin{spverbatim}
% ./greplite --pattern operating --files /home/emran/OS_LAB/test.txt,/home/emran/OS_LAB/test2.txt
% \end{spverbatim}

% \vspace{0.5cm}
% \noindent\textbf{Output:}
% \begin{spverbatim}
% MATCH /home/emran/OS_LAB/test.txt:1:Assalamu-alaikum everyone. How are you? this is test for operating system lab manual.
% OK: MATCHES 1 FILES 2
               

% \end{spverbatim}
\noindent\textbf{Input:}
\begin{lstlisting}[breaklines=true, breakatwhitespace=false, frame=none, backgroundcolor={}, numbers=none, xleftmargin=0pt, columns=fullflexible]
./greplite --pattern operating --files /home/emran/OS_LAB/test.txt,/home/emran/OS_LAB/test2.txt
\end{lstlisting}
...
\section{Process Spawner and Exit-Status Reporter (fork/exec/wait)}
\subsection{Question}
...
int main(int argc, char *argv[]) {
    ...
    // Validation for required command and repeat range
    if (!cmd) {
        fprintf(stderr, "ERROR: E_USAGE: --cmd is required\n");
        return 1;
    }
    if (repeat < 1) {
        fprintf(stderr, "ERROR: E_RANGE: repeat must be >= 1\n");
        return 1;
    }
    ...
    for (int i = 1; i <= repeat; i++) {
        ...
        if (pid < 0) {
            fprintf(stderr, "ERROR: E_FORK: fork failed\n");
            return 1;
        }
        ...
        if (pid == 0) {
            ...
            if (execvp(cmd, exec_args) == -1) {
                // If exec fails, exit with a specific error code
                exit(127); 
            }
        } else {
            ...
            if (waitpid(pid, &status, 0) == -1) {
                fprintf(stderr, "ERROR: E_WAIT: wait failed\n");
                return 1;
            }

            // 3. Report Exit Status
            if (WIFEXITED(status)) {
                int exit_code = WEXITSTATUS(status);
                // Special check for exec failure handled in child
...
\section{Signal-Based Timeout Supervisor (sigaction + alarm + kill)}
\subsection{Question}
...
\section{Pipe-Based Filter Chain (pipe + dup2)}
\subsection{Question}
...
\section{Shared Memory Counter IPC (shm\_open + mmap + sem\_open)}
\subsection{Question}
Shared Memory Counter IPC (shm\_open + mmap + sem\_open)
\subsection{Solution}
\begin{lstlisting}[language=C]
#include <stdio.h>
#include <stdlib.h>
...
\end{verbatim}
\newpage
\section{Threaded Deterministic Reducer (pthreads + mutex)}
\subsection{Question}
...
\end{verbatim}
\newpage
\section{Bounded Buffer Producer-Consumer with Semaphores
...
\end{verbatim}
\newpage
\section{CPU Scheduling Simulator I (FCFS and Non-preemptive SJF)}
\subsection{Question}
...
\end{verbatim}
\newpage
\section{CPU Scheduling Simulator II (Round Robin)}
\subsection{Question}
...
\end{verbatim}
\newpage
\section{Priority Scheduling Simulator (Non-preemptive with Aging)}
\subsection{Question}
...
\end{verbatim}
\newpage
\section{Deadlock Avoidance using Banker's Algorithm}
\subsection{Question}
...
\end{verbatim}
\newpage
\section{Deadlock Detection via Wait-For Graph Cycle}
\subsection{Question}
...
\end{verbatim}
\newpage
\section{Contiguous Memory Allocation Simulator (First/Best/Worst
Fit)}
\subsection{Question}
Contiguous Memory Allocation Simulator (First/Best/Worst
Fit)
\subsection{Solution}
\begin{lstlisting}[language=C]
#include <stdio.h>
...
\newpage
\newpage
\section{Paging Address Translation with Optional TLB}
\subsection{Question}
...
\newpage
\newpage
\section{Page Replacement Simulator (FIFO, LRU, OPT)}
\subsection{Question}
...
\newpage
\newpage
\section{File Allocation Strategy Simulator (Contiguous, Linked,
Indexed)}
\subsection{Question}
File Allocation Strategy Simulator (Contiguous, Linked,
Indexed)
\subsection{Solution}
\begin{lstlisting}[language=C]
#include <stdio.h>
...
\newpage
\newpage
\section{Disk Scheduling Simulator (FCFS, SSTF, SCAN, C-SCAN)}
\subsection{Question}
